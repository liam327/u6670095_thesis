                                 ...
                                 msr::airlib::VehicleSimApiBase* vehicle_sim_api) const
{
    ...
    for (auto i = 0; i < responses.size(); ++i) {
        ...
        //here we are just naming it a frame number 
        image_file_name << "frame_" << frame_number;
    ...
    if (save_success || (responses.size() == 0)) {
        ...
        writeString(vehicle_sim_api->getRecordFileLine(false).append(image_number.str()).append("\n"));
